...
import {
  ArrowLeft, ArrowRight, Building2, ChevronRight, CircleAlert,
...
const labels = {
  gap: 'Coverage gap', active: 'Verified in profile', not_applicable: 'Not applicable'
};
...
function RiskRow({ item, onToggle }) {
  ...
  return (
    <article className="insurance-risk-row">
      <div className="insurance-risk-rank" aria-label={`Priority ${item.priority}`}>{item.priority}</div>
      <div className="insurance-risk-copy">
        <div className="insurance-risk-title"><h3>{item.name}</h3><Status value={item.status} /></div>
        <p>{item.why}</p><small>{item.decision}</small>
      </div>
...
export default function StepInsurance({ profile, updateProfile, nextStep, prevStep, analysis, subStep: controlledPage, setSubStep: setControlledPage }) {
  ...
  if (!lab) return <div className="insurance-loading" role="status"><ShieldCheck size={24} /> Building the protection needs analysis…</div>;
    ...
    <section className="insurance-journey animate-fade-in-up" ref={topRef} aria-labelledby="insurance-title">
      {page === 0 && (
        <header className="insurance-hero">
          ...
          <div className={`insurance-readiness insurance-readiness--${lab.readiness}`}>
            <ShieldCheck size={20} />
            <div>
      ...
      <div className="insurance-page animate-fade-in-up" key={page}>
        ...
        {page === 1 && <div className="insurance-two-column">
          <div className="insurance-panel insurance-panel--focus">
            ...
            <label className="insurance-field"><span>Existing BU benefit <strong>{money(profile.bu_monthly_benefit)}</strong></span><input type="range" min="0" max={Math.max(5000, lab.summary.bu_target_monthly * 1.3)} step="50" value={profile.bu_monthly_benefit || 0} onChange={event => updateProfile({ bu_monthly_benefit: Number(event.target.value) })} /></label>
        ...
        {page === 2 && <div className="insurance-two-column">
          <div className="insurance-panel insurance-panel--focus">
            ...
            <div className="insurance-health-result"><span>{lab.health_decision.eligible_for_comparison ? 'Comparison permitted' : 'GKV compulsory for this profile'}</span><strong>{lab.health_decision.income_headroom >= 0 ? '+' : ''}{money(lab.health_decision.income_headroom)} headroom</strong><p>{lab.health_decision.recommendation}</p></div>
      ...
      <footer className="insurance-footer-nav">
        ...
        {page < pages.length - 1 ? (
          <button type="button" className="insurance-primary-button" onClick={() => move(page + 1)}>
            Continue <ArrowRight size={17} />
          </button>
        ) : (
          <button type="button" className="insurance-primary-button" onClick={nextStep}>
