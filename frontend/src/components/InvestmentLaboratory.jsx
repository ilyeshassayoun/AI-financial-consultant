...
function RiskPage({ answers, answer, recommendation }) {
  ...
  const answerLabels = {
    priority: { growth:'Growth', balanced:'Balanced', income:'Income & real assets', stability:'Stability' },
    lossTolerance: { low:'10% loss budget', medium:'20% loss budget', high:'35%+ loss budget' },
    liquidity: { short:'Within 3 years', medium:'4–9 years', long:'10+ years' },
  };
  return <div className="guide-risk">
    ...
    <div className="guide-risk-review" role="status" aria-live="polite"><div><span>Your suitability inputs</span><strong>{answerLabels.priority[answers.priority]} · {answerLabels.lossTolerance[answers.lossTolerance]} · {answerLabels.liquidity[answers.liquidity]}</strong></div><p>{recommendation ? `Current match: ${recommendation.name}. Continue to apply it, or revisit any answer above.` : 'No growth portfolio passes the current liquidity and loss-capacity constraints.'}</p></div>
...
function StressView({ matrix, profile }) {
  ...
  const [selectedStrategy, setSelectedStrategy] = useState(data[0]?.id || data[0]?.name || '');
  const active = data.find(item => (item.id || item.name) === selectedStrategy) || data[0];
  return <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
    <StressTestFragilityLab profile={profile} initialMode="accumulation" />
    <div className="lab-stress">
      <article className="lab-chart-card lab-chart-card--wide"><div className="lab-chart-title"><div><span>Historical analogues</span><h2>How allocations behave when diversification is needed</h2></div><div className="scenario-badge">Illustrative shocks</div></div><div className="stress-strategy-selector" role="radiogroup" aria-label="Strategy stress details">{data.map(item => <button type="button" role="radio" aria-checked={(item.id || item.name) === selectedStrategy} className={(item.id || item.name) === selectedStrategy ? 'is-active' : ''} key={item.id || item.name} onClick={() => setSelectedStrategy(item.id || item.name)}>{item.name}</button>)}</div><div className="lab-chart lab-chart--stress"><ResponsiveContainer width="100%" height="100%"><BarChart data={data}><CartesianGrid vertical={false} stroke="#e8ece5"/><XAxis dataKey="name" tick={{fontSize:10}} interval={0} axisLine={false}/><YAxis unit="%" axisLine={false}/><Tooltip formatter={v => `${v}%`} contentStyle={{borderRadius:12}}/><Legend/><Bar dataKey="gfc" name="Global financial crisis" fill="#7f3838" radius={[4,4,0,0]} animationDuration={700}/><Bar dataKey="covid" name="Fast equity shock" fill="#b85d52" radius={[4,4,0,0]} animationDuration={850}/><Bar dataKey="rate_shock" name="Rate shock" fill="#c49a55" radius={[4,4,0,0]} animationDuration={1000}/><Bar dataKey="stagflation" name="Stagflation" fill="#7e8f78" radius={[4,4,0,0]} animationDuration={1150}/></BarChart></ResponsiveContainer></div>{active && <div className="stress-selection-detail" role="status"><strong>{active.name}</strong><span>GFC {active.gfc}%</span><span>Fast shock {active.covid}%</span><span>Rate shock {active.rate_shock}%</span><span>Stagflation {active.stagflation}%</span></div>}</article>
