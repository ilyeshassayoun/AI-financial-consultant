...
import {
  ArrowLeft, ArrowRight, BadgeEuro, CalendarClock, CircleAlert,
...
export default function StepRetirement({ profile, updateProfile, nextStep, prevStep, analysis, subStep: controlledPage, setSubStep: setControlledPage }) {
  ...
  if (!lab) return <div className="ret-loading" role="status"><Gauge size={24} /> Building the retirement adequacy lab…</div>;
    ...
    <section className="ret-journey animate-fade-in-up" ref={topRef} aria-labelledby="ret-title">
      {page === 0 && (
        <header className="ret-hero">
          ...
          <div className={`ret-readiness ${lab.readiness.status === 'incomplete' ? 'is-incomplete' : ''}`}>
            <ShieldCheck size={20} />
            <div>
              <span>Record readiness</span>
              <strong>{lab.readiness.status === 'incomplete' ? 'DRV record required' : 'Ready for review'}</strong>
            </div>
          </div>
        </header>
      )}
          ...
          <div className="ret-two-column">
            <article className="ret-panel ret-panel--focus">
              ...
              <div className="ret-form-stack">
                <Field label={`Retirement age · ${profile.retirement_age}`} hint="Early access requires separate eligibility verification.">
                  <input type="range" min={Math.max(profile.age + 1, 60)} max="70" step="1" value={profile.retirement_age} onChange={event => updateProfile({ retirement_age: Number(event.target.value) })} />
                </Field>
                ...
                <Field label={`Target replacement · ${percent(profile.target_pension_ratio)}`} hint="Applied to current modelled monthly net income.">
                  <input type="range" min="0.5" max="1.2" step="0.05" value={profile.target_pension_ratio} onChange={event => updateProfile({ target_pension_ratio: Number(event.target.value) })} />
                </Field>
              </div>
            </article>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={pillars} margin={{ top: 12, right: 12, bottom: 8, left: 8 }}>
                    ...
                    <Bar dataKey="Income" radius={[6, 6, 0, 0]} animationDuration={650}>
                      {pillars.map((item, index) => <Cell key={item.id} fill={colors[index]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
        ...
        {page === 2 && (
          <div className="ret-two-column">
            <article className="ret-panel ret-panel--focus">
              ...
              <div className="ret-big-number"><span>Projected total points</span><strong>{lab.statutory.ep_total}</strong><small>{lab.statutory.ep_past} past + {lab.statutory.ep_future} future points</small></div>
            ...
            <article className="ret-panel">
              ...
              <div className="ret-stat-list">
                ...
                <div><span>Future points / year</span><strong>{lab.statutory.ep_per_future_year}</strong></div>
                <div><span>Access factor</span><strong>{lab.statutory.access_factor.toFixed(3)}</strong></div>
        ...
        {page === 4 && <>
          ...
          <div className="ret-two-column ret-two-column--chart">
            ...
            <article className="ret-panel">
              ...
              <p className="ret-note">The simulation uses {lab.assumptions.simulation_paths.toLocaleString()} seeded paths through age {lab.horizon.longevity_age}. Probability is model sensitivity, not a guarantee.</p>
            </article>
          </div>
        </>}
        ...
        {page === 5 && (
          <article className="ret-policy">
            <div className="ret-policy-header">
              <div><span>{lab.model.name} · {lab.model.version}</span><h2>Retirement funding policy</h2><p>Expressed in today’s euros and prepared from the current profile. Review annually and after any material life event.</p></div>
            ...
            <div className="ret-policy-assumptions">
              <div><span>Selected strategy</span><strong>{lab.assumptions.selected_strategy}</strong></div>
      ...
      <footer className="ret-footer">
        ...
        {page < pages.length - 1 ? (
          <button type="button" className="ret-primary-button" onClick={() => move(page + 1)}>
            Continue <ArrowRight size={17} />
          </button>
        ) : (
          <button type="button" className="ret-primary-button" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
