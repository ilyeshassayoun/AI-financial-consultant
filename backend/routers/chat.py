...
def _chat_context(profile: ClientProfile) -> str:
    analysis = _run_full_analysis(profile)
...
async def consultant_insight(request: Request, payload: ConsultantInsightRequest) -> typing.Any:
    analysis = _run_full_analysis(payload.profile)
...
async def chat_stream_endpoint(request: Request, chat_request: ChatRequest) -> typing.Any:
    context = _chat_context(chat_request.profile)
...
async def chat_endpoint(request: Request, chat_request: ChatRequest) -> ChatResponse:
    return ChatResponse(reply=await generate_financial_advice(chat_request.messages, _chat_context(chat_request.profile)))
